...
class Util {
    ...
    static <T> List<T> jsonToList(JsonArray json, final Function<JsonElement, T> converter) {
        final List<T> result = new ArrayList<>(json.size());
        json.forEach(new Consumer<JsonElement>() {
            @Override
            public void accept(JsonElement j) {
                result.add(converter.apply(j));
            }
        });
        return result;
    }
